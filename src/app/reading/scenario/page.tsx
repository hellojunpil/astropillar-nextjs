...
export default function ScenarioPage() {
  ...
  return (
    <ReadingPageShell
      ...
      subtitle="Get cosmic guidance on a specific situation — ask anything"
      emoji="🔮" badge="2 Credits" badgeColor="#a78bfa" credits={credits} requiredCredits={2}
    >
      {result ? (
        ...
        <div className="card">
          <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 20 }}>
            First, enter your birth details so we can align your chart with your question.
          </p>
          <BirthForm onSubmit={handleBirthSubmit} loading={false} submitLabel="Next → Ask Your Question" />
        </div>
      ) : (
        <form onSubmit={handleQuestionSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card">
            <p style={{ color: 'var(--gold)', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 12 }}>
              Your Question
            </p>
            <textarea
              value={question}
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {EXAMPLE_QUESTIONS.map((q, i) => (
                  <button key={i} type="button" onClick={() => setQuestion(q)} style={{
                    textAlign: 'left', background: 'none', border: '1px solid var(--border)',
                    borderRadius: 8, padding: '8px 12px', color: 'var(--text-muted)', fontSize: 12, cursor: 'pointer',
                  }}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button type="button" onClick={() => setStep('form')} style={{
              ...
              color: 'var(--text-muted)', borderRadius: 50, padding: '13px 20px', fontSize: 13, cursor: 'pointer',
            }}>
              ← Back
            </button>
            <button type="submit" disabled={!question.trim() || submitting} className="btn-gold" style={{
              flex: 1, opacity: (!question.trim() || submitting) ? 0.5 : 1,
