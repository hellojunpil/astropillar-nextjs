...
function CardBack({ size = 'md' }: { size?: 'sm' | 'md' }) {
  const dims = size === 'sm' ? { w: 44, h: 66, fs: 14 } : { w: 68, h: 102, fs: 20 }
  return (
    <div style={{
          ...
          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            {revealed && card
              ...
              : <div style={{ width: 80, height: 120, background: 'linear-gradient(135deg,#16213E,#0f1829)', border: '1.5px solid rgba(201,168,76,0.6)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ color: 'rgba(201,168,76,0.7)', fontSize: 22 }}>✦</span>
                </div>
            }
...
export default function RelationshipPage() {
  ...
  async function startReading() {
    ...
    try {
      ...
      if (user?.email) {
        await Promise.all([
          apiPost('/use_pouch', { email: user.email, reading_type: 'tarot_relationship' }),
          ...
          <div>
            ...
            <div className="card" style={{ padding: '0 20px', marginBottom: 20 }}>
              {parseResult(gptText)
                ...
                return <Section key={i} title={localizeTarotTitle(sec.title ?? `Section ${i + 1}`, locale)} content={sec.content} defaultOpen={i === 4} />
              })}
            </div>
            ...
            <TarotShareButton userEmail={user?.email ?? ''} share={gptText ? {
              reading_type: 'tarot_relationship',
              name: question.trim() || 'Relationship Reading',
              result: {
